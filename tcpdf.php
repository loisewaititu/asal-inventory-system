	...
	public function Header() {
		if ($this->header_xobjid === false) {
			...
			if ($this->getRTL()) {
				...
				$header_x = $this->original_lMargin + (floatval($headerdata['logo_width']) * 1.1);

			}
			$cw = $this->w - $this->original_lMargin - $this->original_rMargin - (floatval($headerdata['logo_width']) * 1.1);
